...
/* Returns  : Nothing                              */
/*-------------------------------------------------*/
void PrintXY(byte x, byte y, byte wAttr, char Chr)
{
...
void WriteChar(byte x, byte y, byte wAttr, byte bLen, char Chr)
{
	...
	for(i = 1; i <= bLen; i++) {
		fpVRM[rwVideoOFS    ] = Chr;
		fpVRM[rwVideoOFS + 1] = wAttr;
		rwVideoOFS += 2;
	}
}
...
/* Returns : Nothing                             */
/*-----------------------------------------------*/
void WriteVRM(byte x, byte y, byte txtAtr, const char *szPrmt, byte fstAttr = 0)
{
...
/* Returns : Nothing                             */
/*-----------------------------------------------*/
void Button(byte x, byte y, byte txtAttr, byte bckClr, const char *szTitle, byte bType = 0, byte fstAttr = 0)
{
...
/* Returns  : Nothing                                */
/*---------------------------------------------------*/
void ChangeAttr(byte x1, byte y1, byte x2, byte y2, byte wAttr)
{
	register byte bCol, bRow;

	for(bCol = x1; bCol <= x2; bCol++)
		for(bRow = y1; bRow <= y2; bRow++)
			fpVRM[OFFSET(bCol, bRow) + 1] = wAttr;
}
...
/* Returns  : Nothing                           */
/*----------------------------------------------*/
void DlgBox(byte x1, byte y1, byte x2, byte y2, byte wAttr)
{
	byte oldAttr;
...
/* Returns  : Nothing                           */
/*----------------------------------------------*/
void DlgWin(byte x1, byte y1, byte x2, byte y2, byte wAttr, char *szTitle)
{
...
void HaltSys()
{
	...
	strcpy(szPath,SysInf[23]); strcat(szPath, "off");
...
{
	char szPath[32];
	GetData("register.txt","register.$$$",SysInf,bnInfo);
	strcpy(szPath,SysInf[23]); strcat(szPath, "on"); system(szPath);
}
